...
.. _FX4.cmd:              https://github.com/epics-modules/quadEM/tree/master/iocBoot/iocFX4/FX4.cmd
...
These meters communicate via IP, so they must be configured with an IP address reachable
from the host IOC machine. The FX4 uses DHCP by default.  Once connected the Network tab
on the FX4 Web page can be used to change to a static IP if desired.

An example startup script is provided in FX4.cmd_.
