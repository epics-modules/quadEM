...

/** Constructor for the drvNSLS2_EM class.
  ...
  */
drvNSLS2_EM::drvNSLS2_EM(const char *portName, const char *broadcastAddress, int moduleID, int ringBufferSize) 
...
//    unlock();
    ...
    status = (asynStatus)(epicsThreadCreate("drvNSLS2_EMTask",
...
static void readThread(void *drvPvt)
{
    drvNSLS2_EM *pPvt = (drvNSLS2_EM *)drvPvt;
    
    pPvt->readThread();
}

asynStatus drvNSLS2_EM::findModule()
{
    size_t nwrite;
    size_t nread;
...
asynStatus drvNSLS2_EM::writeReadMeter()
{
  size_t nread;
  size_t nwrite;
...
void drvNSLS2_EM::readThread(void)
{
    asynStatus status;
    size_t nRead;
    int eomReason;
...
/** Set the acquisition mode
  */
asynStatus drvNSLS2_EM::setMode()
{
    int pingPong;
    int mode;
...
/** Sets the integration time. 
  ...
  */
asynStatus drvNSLS2_EM::setIntegrationTime(epicsFloat64 value) 
{
    asynStatus status;
...
asynStatus drvNSLS2_EM::setValuesPerRead(epicsInt32 value) 
{
  ...
  */
asynStatus drvNSLS2_EM::setPingPong(epicsInt32 value) 
{
    return setMode();
}
...
/** EPICS iocsh callable function to call constructor for the drvNSLS2_EM class.
  * \param[in] portName The name of the asyn port driver to be created.
  * \param[in] broadcastAddress The broadcast address of the network with this module
  * \param[in] moduleID The module ID of this module, set with rotary switch on module
  * \param[in] ringBufferSize The number of samples to hold in the input ring buffer.
  ...
  *            If 0 then default of 2048 is used.
  */
int drvNSLS2_EMConfigure(const char *portName, const char *broadcastAddress, int moduleID, int ringBufferSize)
{
    new drvNSLS2_EM(portName, broadcastAddress, moduleID, ringBufferSize);
    return(asynSuccess);
}
...
static const iocshArg * const initArgs[] = {&initArg0,
                                            ...
                                            &initArg2,
                                            &initArg3};
static const iocshFuncDef initFuncDef = {"drvNSLS2_EMConfigure",4,initArgs};
static void initCallFunc(const iocshArgBuf *args)
{
    drvNSLS2_EMConfigure(args[0].sval, args[1].sval, args[2].ival, args[3].ival);
}
